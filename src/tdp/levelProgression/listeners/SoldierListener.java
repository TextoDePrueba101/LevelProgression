...
public class SoldierListener implements Listener{
	...
	public void onDamage(EntityDamageByEntityEvent e) {
		...
		if (!(e.getEntity() instanceof LivingEntity));
			...
			boolean secondIsSword=false;
			if (isSword(p, p.getInventory().getItemInMainHand())) {
				secondIsSword = true;
				swordVerifier(p, p.getInventory().getItemInMainHand());
			}
			if (isSword(p, offHand) && p.getAttackCooldown()==1) {
				...
				if (!secondIsSword);
				else if (offHand.getType()== Material.NETHERITE_SWORD && LevelProgression.pgetData(p, "SOLDIER") >= 250) {				
					damm=damm+4;
					if (abss<10) abss += 1;
					p.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 20*2, 0, true, true));
					...
					damm=damm+3;
					if (abss<8) abss += 1;
					p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_FALLING, 15, 0, true, true));
					...
					damm=damm+2;
					if (abss<6) abss += 1;
					p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 20*2, 0, true, true));
					...
					damm=damm+1;
					if (abss<4) abss += 1;
					p.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 20*2, 0, true, true));

					damageItem(p, offHand, 1);
				}
				else if (offHand.getType()== Material.GOLDEN_SWORD) {				
					if (abss<2) abss += 1;
					p.addPotionEffect(new PotionEffect(PotionEffectType.LUCK, 20*2, 2, true, true));
					
					damageItem(p, offHand, 1);
				}
				else if (offHand.getType()== Material.WOODEN_SWORD) {				
					if (abss<2) abss += 1;
				}
				...
				if (LevelProgression.pgetData(p,"SOLDIER") >= 50 && mainHand.getType() == Material.WOODEN_SWORD) {
					...
					if (ent instanceof LivingEntity) {
						...
						if (ent1.getLocation().distance(p.getLocation()) < 10) {
							...
							launchVector.normalize().multiply(1.0);
					...
					if (ent instanceof LivingEntity) {
						LivingEntity ent1 = (LivingEntity) ent;
						if (ent1.getLocation().distance(p.getLocation()) < 6) {
							p.attack(ent1);
						}
					}
					...
					for (Entity entities : ent) {
						if (entities instanceof LivingEntity) {
							...
							if (entity.getLocation().distance(p.getLocation()) < 7 && entity!=p) {
								...
								Vector launchVector = loc2.toVector().subtract(loc1.toVector());
								launchVector.normalize().multiply(0.9);
