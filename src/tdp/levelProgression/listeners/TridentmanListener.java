	...
	public void specialTridentHit(ProjectileHitEvent e) {
		...
		if (trident.getCustomName().equals("NETHERITE_INGOT TRIDENT")) {
			...
			for (Entity entities : ent) {
				if (entities instanceof Item) entities.teleport(((Player)((Projectile)(Trident)trident).getShooter()).getLocation());
			...
			if (e.getHitEntity()!=null && (e.getHitEntity() instanceof LivingEntity)) {
				...
				ent.setVelocity(new Vector(0,.8,0));
				ent.setVelocity(ent.getVelocity().add(trident.getVelocity().normalize().setY(0).normalize().multiply(-1.2)));
