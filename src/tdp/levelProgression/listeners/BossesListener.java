...
public class BossesListener implements Listener{
	...
	public void SummonBoss(EntityDeathEvent e) {
		...
		&& e.getEntity().getKiller() != null && e.getEntity().getLastDamageCause().getCause()==DamageCause.ENTITY_ATTACK) {
			...
			if (isBoss(e.getEntity())) {
				...
				try {
					if (e.getEntity().getKiller()!= null || boss.getLastDamageCause().getCause().equals(DamageCause.PROJECTILE)) {
						...
						if (boss.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue() >= 200) {
							if ((Math.random()*100+1)<=50) for (ItemStack i : specialItem(0,boss)) e.getDrops().add(i);
							else for (ItemStack i : specialItem(1,boss)) e.getDrops().add(i);
						}
					}
				}
				catch(Exception ex) {}
	...
	public Collection<ItemStack> specialItem(int n,LivingEntity boss) {
		ItemStack item = new ItemStack(Material.STICK);
		ItemMeta m = item.getItemMeta();
		int ammount=1;
		Collection<ItemStack> finalDrop = new ArrayList<ItemStack>();
		ArrayList<String> lore = new ArrayList<String>();
		...
		for (int i=0;i<ammount;i++) finalDrop.add(item);
	...
	public void BossesAbilitiesAtDamageOfEntity(EntityDamageByEntityEvent e) {
		...
		&& !isBoss(e.getDamager())) {
			...
			((LivingEntity)e.getDamager()).setHealth(0);
		}
		...
		&& isBoss(e.getDamager())) {
			...
			p.addPotionEffect(new PotionEffect(PotionEffectType.HUNGER,20* 3,1,true));
		}
	...
	public boolean isBoss(Entity ent) {
		if (ent.getPersistentDataContainer().has(new NamespacedKey(LevelProgression.getPlugin(LevelProgression.class), "boss"), PersistentDataType.INTEGER)) return true;
		return false;
	}
	...
	public void ColdSkeletonHit(ProjectileHitEvent e) {
		...
		&& ((LivingEntity) e.getHitEntity()).getPersistentDataContainer().has(new NamespacedKey(LevelProgression.getPlugin(LevelProgression.class), "boss"), PersistentDataType.INTEGER)) {
			...
			if (arrow.getShooter() instanceof Player) {
				...
				if (!isBoss(badEnemy)) badEnemy.setHealth(0);
				else try{
					badEnemy.setHealth(badEnemy.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue()+30);
					boss.setHealth(boss.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue()+30);
				}
				catch(Exception ex) {}
			}
		}
	...
	public void ColdSkeletonShoot(EntityShootBowEvent e) {
		...
		&& isBoss(e.getEntity())) {
			...
			if ( boss.getHealth()+addHp < boss.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue()) boss.setHealth(boss.getHealth()+addHp);
			...
			if ((r*100+1) <= 10) {
				...
				spawn(boss.getLocation(),EntityType.SPIDER,0,6,2);
				spawn(boss.getLocation(),EntityType.HUSK,5,10,4);
			}
			else if ((r*100+1) <= 35) {
		    ...
		    @Override
		     public void run(){
			    ...
			    if ((Math.random()*1000+1)<=5) {
			    	   ...
			    	   spawnWater(boss.getLocation(),EntityType.GUARDIAN,0,20, (playerCount*3));
			    }
	...
	public void DryBoss(Location loc, int lvl) {
		...
		if (testHealth>2000) {
			...
			boss.getAttribute(Attribute.GENERIC_ARMOR).setBaseValue(additionalArmour);
			boss.getAttribute(Attribute.GENERIC_ARMOR_TOUGHNESS).setBaseValue(additionalToughness);
			boss.getAttribute(Attribute.GENERIC_KNOCKBACK_RESISTANCE).setBaseValue(40);
		...
		boss.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 20* 99999, 1, true));
		    ...
		    @Override
		     public void run(){
			    ...
			    if ((Math.random()*1000+1)<=5) {
			    	...
			    	int playerCount=0;
			    	for (Entity ent : entities) if (ent instanceof Player) playerCount++;
			    	spawn(boss.getLocation(),EntityType.CAVE_SPIDER,0,20,(playerCount+1) * 2 +4);
			    	spawn(boss.getLocation(),EntityType.SILVERFISH,0,20, (playerCount+1) * 2 +4);
			    }
			    
			    if ((Math.random()*1000+1)<=5) {
					boolean tp=true;
					List<Entity> near = boss.getNearbyEntities(55, 55, 55);
			    ...
			    if ((Math.random()*1000+1)<=6) {
			    	List<Entity> entities = boss.getNearbyEntities(55, 55, 55);
